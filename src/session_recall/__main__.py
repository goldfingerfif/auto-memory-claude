...
def main() -> None:
    ...
    t0 = time.monotonic()
    parser = argparse.ArgumentParser(prog="auto-memory", description="Query session history")
    parser.add_argument("--backend", choices=sorted(backends.REGISTRY.keys()), default=None,
    ...
    p_ingest = sub.add_parser("ingest", help="Refresh the Claude Code index from JSONL transcripts")
